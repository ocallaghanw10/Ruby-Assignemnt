...
# on the screen (starting from 1,1 for the top-left corner).
  ...
  if $y >= 0 && $x >= 0 && $y < SCREEN_Y && $x < SCREEN_X
    ...
    $oldx=$x.floor;$oldy=$y.floor
      ...
      #prints the racquet
      print "\e[#{16};#{$oldracquet}H----"
      print "\e[#{16};#{racquet}H===="
      $oldracquet = racquet
  ...
  # clears the old position of the racquet, using the value in the screen array
  # and plots the current position.
...
(0...SCREEN_Y).each do |row|
		(0...SCREEN_X).each do |column| 
		  if column == SCREEN_Y - 1
		    puts "|"
              elsif screen[row][column] == SC_H
              print SC_H
              elsif screen[row][column] == SC_V
              print SC_V
               elsif screen[row][column] == SC_BLANK
              print SC_BLANK
...
# "racquet_hit" for a ball reflecting from a racquet
